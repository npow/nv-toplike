...
pub fn render_memory(frame: &mut Frame<'_>, area: Rect, app: &App, device: &DeviceSnapshot) {
    ...
    frame.render_widget(
        Paragraph::new(header_line).block(panel(
            " Memory Foundry · Hardware Memory Hierarchy & Allocation Flow ",
            PINK,
        )),
        main_layout[0],
    );
    ...
    frame.render_widget(
        Paragraph::new(
            "Hardware telemetry: PCIe rates and VRAM fill are direct NVML measurements. Internal L2/SM motion is architectural context.",
        )
...
fn render_foundry_canvas(frame: &mut Frame<'_>, area: Rect, app: &App, device: &DeviceSnapshot) {
...
fn render_vram_block_map(frame: &mut Frame<'_>, area: Rect, app: &App, device: &DeviceSnapshot) {
    let fill_ratio = device.memory_fill_ratio().unwrap_or(0.0);
    ...
    for row in 0..inner_height {
        ...
        for col in 0..inner_width {
            ...
            if cell_idx < filled_total {
                let pulse = (app.frame.wrapping_add(cell_idx as u64 * 7)).is_multiple_of(11)
                    && mem_activity > 0.05;
                if pulse {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{used_fmt} Used ({:.1}%)", fill_ratio * 100.0),
            Style::default().fg(AMBER).add_modifier(Modifier::BOLD),
        ),
        Span::raw(" │ Free: "),
...
fn render_l2_cache_matrix(frame: &mut Frame<'_>, area: Rect, app: &App, device: &DeviceSnapshot) {
    ...
    lines.push(Line::from(Span::styled(
        format!(
            "L2 Cache Crossbar Array (16 Partitions) · Crossbar Bus Activity {:.0}%",
            activity * 100.0
        ),
        Style::default().fg(AMBER),
    )));
...
fn render_sm_vaults_matrix(frame: &mut Frame<'_>, area: Rect, app: &App, device: &DeviceSnapshot) {
    ...
    lines.push(Line::from(Span::styled(
        format!(
            "SM Execution Vaults · L1 Shared Memory & Register Files · Activity {:.0}%",
            visual_act * 100.0
        ),
...
fn render_memory_telemetry_deck(frame: &mut Frame<'_>, area: Rect, device: &DeviceSnapshot) {
    ...
    if device.processes.is_empty() {
        proc_lines.push(Line::from(Span::styled(
            "No active process VRAM allocations",
    ...
    frame.render_widget(
        Paragraph::new(proc_lines).block(panel(" Process VRAM Attribution ", CYAN)),
        chunks[3],
    );
}
