...
fn render_cluster_summary(frame: &mut Frame<'_>, area: Rect, snapshot: &Snapshot) {
    ...
    let summary_line = Line::from(vec![
        ...
        Span::raw("  "),
        Span::styled(format!("{device_count} Device{}", if device_count == 1 { "" } else { "s" }), Style::default().fg(GOLD).add_modifier(Modifier::BOLD)),
        Span::raw(" · Cluster VRAM: "),
        Span::styled(
            format!("{} / {}", fmt_bytes(Some(total_vram_used)), fmt_bytes(Some(total_vram_cap))),
        ...
        Span::styled(
            fmt_temp(avg_temp),
            Style::default().fg(heat_color(avg_temp.unwrap_or(35.0))).add_modifier(Modifier::BOLD),
        ),
    ]);

    frame.render_widget(
        Paragraph::new(summary_line)
            .block(panel(" Fleet Overview ", CYAN)),
        area,
    );
}
...
fn render_fleet_table(frame: &mut Frame<'_>, area: Rect, app: &App, snapshot: &Snapshot) {
    let rows = snapshot.devices.iter().enumerate().map(|(index, device)| {
        ...
        let sm_str = device.device.compute_units.map_or_else(|| "-".to_owned(), |c| c.to_string());
...
fn render_fleet_cards(frame: &mut Frame<'_>, area: Rect, app: &App, snapshot: &Snapshot) {
    ...
    for (index, device) in snapshot.devices.iter().take(cols).enumerate() {
        ...
        lines.push(Line::from(vec![
            Span::styled(format!("GPU {}: ", device.device.display_index.unwrap_or(index as u32)), Style::default().fg(GOLD).add_modifier(Modifier::BOLD)),
            Span::styled(short_name(&device.device.name, 16), Style::default().fg(WHITE)),
        ]));

        // Mini SM bar
        let sm_bar_width = 10;
        let filled_sm = (gpu_ratio * sm_bar_width as f64).round() as usize;
        let sm_bar: String = (0..sm_bar_width).map(|i| if i < filled_sm { '▓' } else { '░' }).collect();
        lines.push(Line::from(vec![
            Span::styled("GPU  ", Style::default().fg(MUTED)),
            Span::styled(sm_bar, Style::default().fg(util_color(gpu_ratio))),
            Span::styled(format!(" {:>3.0}%", gpu_ratio * 100.0), Style::default().fg(CYAN)),
        ]));

        // Mini VRAM bar
        let filled_mem = (mem_fill * sm_bar_width as f64).round() as usize;
        let mem_bar: String = (0..sm_bar_width).map(|i| if i < filled_mem { '▓' } else { '░' }).collect();
        lines.push(Line::from(vec![
            Span::styled("VRAM ", Style::default().fg(MUTED)),
            Span::styled(mem_bar, Style::default().fg(AMBER)),
            Span::styled(format!(" {:>3.0}%", mem_fill * 100.0), Style::default().fg(AMBER)),
        ]));
        ...
        frame.render_widget(
            Paragraph::new(lines)
                .block(panel(&format!(" Device {} Card ", index), border_color)),
            card_chunks[index],
        );
    }
}
